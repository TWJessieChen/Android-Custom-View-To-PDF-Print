package com.jc666.customviewtopdfprint.ecgview;
...
public class ScrollECGView extends View {
    ...
    public ScrollECGView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //  背景色
        this.setBackgroundColor(Color.parseColor("#ff000000"));
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            Log.d("json", "本页面宽： " + width + "  高:" + height + " data_num: " + data_num);
//            Log.e("json","两点间横坐标间距:" + gap_x + "   矩形区域两点间横坐标间距：" + rect_gap_x);
    ...
    private void DrawGrid(Canvas canvas) {
        ...
        for (int i = 1; i < grid_hori + 2; i++) {
            ...
            paint.setColor(Color.parseColor("#ff949497")); //<color name="data_pr">#0a7b14</color>
        ...
        for (int i = 1; i < grid_ver + 2; i++) {
            Paint paint = new Paint();
            paint.setStyle(Paint.Style.STROKE);
            paint.setColor(Color.parseColor("#ff949497"));
            paint.setStrokeWidth(1.0f);
            Path path = new Path();
    ...
    private void DrawECGWave(Canvas canvas) {
        Log.e("json", "DrawECGWave");
        Paint paint = new Paint();
        paint.setStyle(Paint.Style.STROKE);
        paint.setColor(Color.parseColor("#ff00fb00"));
        ...
        Rect_paint.setColor(Color.parseColor("#ff000000"));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                startX = event.getX();
                Log.e("json", "startX" + startX);
                break;

            case MotionEvent.ACTION_MOVE:
                x_change = event.getX() - startX;
                Log.e("json", "startX" + x_change);
                invalidate();
                break;
        }
        return true;
     ...
     */
    private float getY_coordinate(Integer data) {
    ...
    public void setData(ArrayList<Integer> data) {
        this.data_source = (ArrayList<Integer>) data.clone();
        invalidate();
    }
